...
const WarehouseContext = React.createContext<WarehouseContextType | undefined>(undefined);

export const useWarehouseContext = () => {
    const ctx = React.useContext(WarehouseContext);

    if (ctx === undefined) {
        throw new Error(`'useWarehouseContext' must be used within a 'WarehouseContextProvider'`);
    }
...
export const WarehouseProvider = ({ warehouse, children }: IWarehouseProvider) => {
