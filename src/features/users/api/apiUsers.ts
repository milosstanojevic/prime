...
export const useGetUser = (id: number) => useFetch<User>(pathToUrl(singleEntityUrl, { id }));

export const useEditUser = (updater: (oldData: User[], newData: User) => User[]) =>
    usePut<User[], User>(pathToUrl(mainEntityUrl), undefined, updater);

export const useDeleteUser = (updater: (oldData: User[], deletedId: string | number) => User[]) =>
