...
export const addTransportOrderArticle = (
  id: number,
  ...
  try {
    ...
    dispatch(
      actions.transportOrderArticleSuccess(
        response as NormalizerTransportOrderArticleRequest
      )
    );
  } catch (error) {
...
export const fetchTransportOrderArticles = createAsyncThunk(
  "transportOrderArticles/fetchTransportOrderArticles",
  async (id: number, { dispatch }) => {
    const response: Object = await request(`transport-orders/${id}/articles`, {
      schema: schemas.TRANSPORT_ORDER_ARTICLES,
    });
    dispatch(
      actions.transportOrderArticlesSuccess(
        response as NormalizerTransportOrderArticlesRequest
      )
    );
    return response;
  }
...
export const fetchTransportOrderArticle = (
  id: number,
  articleId: number
): AppThunk => async (dispatch: AppDispatch) => {
  ...
  try {
    const response: Object = await request(
      `transport-orders/${id}/articles/${articleId}`,
      {
        schema: schemas.TRANSPORT_ORDER_ARTICLE,
      }
    );
...
export const updateTransportOrderArticle = (
  id: number,
  articleId: number,
  attributes: TransportOrderArticle
): AppThunk => async (dispatch: AppDispatch) => {
...
export const deleteTransportOrderArticle = (
  id: number,
  articleId: number
): AppThunk => async (dispatch: AppDispatch) => {
  try {
    ...
    dispatch(
      actions.transportOrderArticleRemoved(
        response as NormalizerTransportOrderArticleRequest
      )
    );
  } catch (error) {
